...
public class Mayor : CustomRole
{
    ...
    public static void AddMayorButton(PlayerVoteArea __instance)
    {
        ...
        var voteTwiceButton =
            Object.Instantiate(__instance.ConfirmButton, __instance.ConfirmButton.transform.parent, true);
        voteTwiceButton.name = "VoteTwiceButton";
        voteTwiceButton.transform.FindChild("ControllerHighlight").GetComponent<SpriteRenderer>().color =
            CustomRoleManager.GetRole<Mayor>().Color;
        ...
        button.AddOnClickListeners(new Action(() =>
        {
        ...
        if (!__instance.voteComplete && __instance.Parent.Select(__instance.TargetPlayerId))
        {
            ...
            var effects = new List<IEnumerator>
            {
                Effects.Lerp(0.25f,
                    new Action<float>(t =>
                    {
                        __instance.CancelButton.transform.localPosition = Vector2.Lerp(Vector2.right * startPos,
                            Vector2.right * 1.3f, Effects.ExpOut(t));
                    })),
                Effects.Lerp(0.35f,
                    new Action<float>(t =>
                    {
                        __instance.ConfirmButton.transform.localPosition = Vector2.Lerp(Vector2.right * startPos,
                            Vector2.right * 0.65f, Effects.ExpOut(t));
                    }))
            };
            if (voteTwiceButton != null)
                effects.Add(Effects.Lerp(0.45f,
                    new Action<float>(t =>
                    {
                        voteTwiceButton.transform.localPosition = Vector2.Lerp(Vector2.right * startPos,
                            Vector2.right * 0f, Effects.ExpOut(t));
                    })));
            ...
            if (voteTwiceButton != null) selectableElements.Add(voteTwiceButton.GetComponent<UiElement>());
            ControllerManager.Instance.OpenOverlayMenu(__instance.name, __instance.CancelButton,
                __instance.ConfirmButton, selectableElements.WrapToIl2Cpp());
            return false;
        }
