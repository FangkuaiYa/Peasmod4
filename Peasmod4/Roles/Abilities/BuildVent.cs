...
public class BuildVent
{
    ...
    private static int GetVentId()
    {
        return ShipStatus.Instance.AllVents.Count;
    }
    ...
    public static void CreateVent(Vector3 position)
    {
        ...
        if (LastPlacedVent != int.MinValue) leftVent = LastPlacedVent;

        vent.Left = leftVent == int.MinValue
            ? null
            : ShipStatus.Instance.AllVents.FirstOrDefault(v => v.Id == leftVent);
        vent.Center = null;
        vent.Right = null;
