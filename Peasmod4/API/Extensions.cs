...
public static class Extensions
{
    ...
    public static bool IsLocal(this NetworkedPlayerInfo player)
    {
        return IsLocal(player.Object);
    }
    ...
    public static PlayerControl FindNearestPlayer(this PlayerControl player, Predicate<PlayerControl> selector,
        float distance = 3f, bool ignoreColliders = false)
    {
        ...
        foreach (var target in PlayerControl.AllPlayerControls)
            if (selector.Invoke(target))
            {
                ...
                if (magnitude <= distance && (ignoreColliders ||
                                              !PhysicsHelpers.AnyNonTriggersBetween(myPos, vector.normalized, magnitude,
                                                  Constants.ShipAndObjectsMask))) outputList.Add(target);
            }

        outputList.Sort(delegate(PlayerControl a, PlayerControl b)
        {
            var magnitude2 = (a.GetTruePosition() - myPos).magnitude;
            var magnitude3 = (b.GetTruePosition() - myPos).magnitude;
            if (magnitude2 > magnitude3) return 1;
            if (magnitude2 < magnitude3) return -1;
            return 0;
        });
        ...
        float distance = 3f, bool ignoreColliders = false)
    {
        ...
        foreach (var collider in Physics2D.OverlapCircleAll(myPos, biggestDistance))
        {
            ...
            if ((obj.GetComponent<SpriteRenderer>() != null || obj.GetComponentInChildren<SpriteRenderer>() != null) &&
                selector.Invoke(obj))
            {
                var vector = new Vector2(obj.transform.position.x, obj.transform.position.y) - myPos;
                var magnitude = vector.magnitude;
                if (magnitude <= biggestDistance && (ignoreColliders ||
                                                     !PhysicsHelpers.AnyNonTriggersBetween(myPos, vector.normalized,
