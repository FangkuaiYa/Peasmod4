...
public class CustomButton
{
    ...
    public CustomButton(string objectName, Action onClick, string text, Sprite image, Predicate<PlayerControl> couldUse,
        Predicate<PlayerControl> canUse, CustomButtonOptions options = null)
    {
    ...
    public void Start(object sender, EventArgs args)
    {
        ...
        Button = GameObject.Instantiate(HudManager.Instance.AbilityButton,
            Options.OnLeftSide
                ? Patches.LeftBottomParent.transform
        ...
        if (CouldBeUsed() && PlayerControl.LocalPlayer.IsKillTimerEnabled)
        {
            ...
            else if (Options._TargetType == CustomButtonOptions.TargetType.Object)
            {
                ...
                if (ObjectTarget && ObjectTarget != newTarget)
                {
                    ...
                    if (image) image.material.SetFloat("_Outline", 0);
                }
    ...
    public void SetCooldown(float cooldown, float maxCooldown = float.NaN)
    {
        Cooldown = cooldown;
